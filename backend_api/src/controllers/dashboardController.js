...
export async function summary(req, res) {
  ...
  const revenue = completed.reduce((sum, item) =>
    sum + Number(item.received_amount ?? item.services?.price ?? 0), 0);
  const byBarber = barbers.map((barber) => {
    const rows = completed.filter((item) => item.barber_id === barber.id);
    const visits = rows.filter((item) => !isInternalPayment(item));
  ...
  const todayRows = appointments.filter((item) =>
    item.date === today && !isInternalPayment(item));
  ...
  res.json({
    month,
    appointments: appointments.filter((item) => !isInternalPayment(item)).length,
    ...
    today: {
      ...
      revenue: todayCompleted.reduce((sum, item) =>
        sum + Number(item.received_amount ?? item.services?.price ?? 0), 0),
      nextAppointment,
    },
    pending,
    walletCount: walletRows.length,
    walletAmount: walletRows.reduce((sum, item) =>
      sum + Number(item.received_amount ?? item.services?.price ?? 0), 0),
